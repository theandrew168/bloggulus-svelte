...
import type { Config, OAuthConfig } from "$lib/server/config";
...
export class TOMLConfig implements Config {
	private static _instance?: TOMLConfig;
	...
	static getInstance(): TOMLConfig {
		if (!this._instance) {
			...
			this._instance = new TOMLConfig(body);
		}
